...
def add_message(username, content):
    ...
    try:
        with (connection := get_db_connection()):
            connection.execute("INSERT INTO messages (username, content) VALUES (?,?)",
             (username, content))
            connection.commit()

    except sqlite3.Error as exception:
        print(f"Error while adding message with username {username!r} "
...
def contact(num_contacts = 1):
    ...
    phone_numbers = [f"({randint(100,999)})-{randint(100,999)}-{randint(1000,9999)}"
        for i in range(num_contacts)]

    return render_template('contact.j2', phone_numbers=phone_numbers,
     username=session.get('username'))
